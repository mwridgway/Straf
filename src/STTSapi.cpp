...
private:
    void Run(){
        ...
        if (FAILED(CoCreateInstance(CLSID_SpSharedRecognizer, nullptr, CLSCTX_ALL, IID_PPV_ARGS(&recognizer_)))){ return; }
        if (FAILED(recognizer_->CreateRecoContext(&recog_))){ return; }
        if (FAILED(recog_->SetNotifyCallbackInterface(this, 0, 0))){ return; }
        ULONGLONG interests = SPFEI(SPEI_RECOGNITION);
        if (FAILED(recog_->SetInterest(interests, interests))){ return; }
        if (FAILED(recognizer_->SetRecoState(SPRST_ACTIVE_ALWAYS))){ return; }
        if (FAILED(recog_->SetAudioOptions(SPAO_RETAIN_AUDIO, nullptr, nullptr))){ /* optional */ }

        // Use dictation
        if (FAILED(recog_->CreateGrammar(1, &grammar_))){ return; }
        if (FAILED(grammar_->LoadDictation(nullptr, SPLO_STATIC))){ return; }
        if (FAILED(grammar_->SetDictationState(SPRS_ACTIVE))){ return; }
